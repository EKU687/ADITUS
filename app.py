...
def init_connection() -> Client:
    """Initialise la connexion à Supabase en utilisant les secrets de Streamlit."""
...
def charger_sites_refero():
    ...
    try:
        ...
        liste_brute = [ligne["code_site"] for ligne in reponse.data if ligne.get("code_site")]
...
def envoyer_email_notification(destinataire_email, site_nom, demandeur_email, motif_demande):
    ...
    try:
        ...
        st.warning(f"⚠️ La demande a été enregistrée, mais la notification email n'a pas pu être envoyée : {e}")
        return False

def envoyer_email_decision(destinataire_email, site_nom, decision, date_entree=None, heure_entree=None, date_sortie=None, heure_sortie=None, motif_refus=None):
    ...
    try:
        ...
        d_ent_fr = f"{date_entree[8:10]}/{date_entree[5:7]}/{date_entree[0:4]}" if date_entree and len(date_entree) >= 10 else "N/C"
        d_sor_fr = f"{date_sortie[8:10]}/{date_sortie[5:7]}/{date_sortie[0:4]}" if date_sortie and len(date_sortie) >= 10 else "N/C"
        ...
        st.warning(f"⚠️ La décision a été enregistrée, mais l'e-mail au demandeur n'a pas pu être envoyé : {e}")
        return False
...
def verifier_roles_utilisateur(email_utilisateur):
    ...
    # Liste des e-mails administrateurs (à adapter selon tes besoins)
    ...
    try:
        # On vérifie si l'e-mail existe dans la table des valideurs
        req = supabase.table("Valideurs_sites").select("valideur_email").eq("valideur_email", email_utilisateur).execute()
        if len(req.data) > 0:
            est_valideur = True
    except Exception as e:
...
if not st.session_state["user_authenticated"]:
    ...
    st.markdown("Veuillez vous authentifier avec votre adresse e-mail professionnelle pour accéder aux services.")
    st.divider()
    ...
    with col_login:
        ...
        if not st.session_state["otp_sent"]:
            ...
            email_saisi = st.text_input("Adresse e-mail professionnelle", placeholder="prenom.nom@gouv.nc")
            
            if st.button("Envoyer le code de vérification 📩", type="primary", width="stretch"):
                if email_saisi and "@" in email_saisi:
                    try:
                        ...
                        st.success(f"Un code de vérification a été envoyé à **{email_saisi}**.")
                        st.rerun()
                    except Exception as e:
        ...
        # Étape 2 : Saisie du code à 8 chiffres
        else:
            st.subheader("📩 Saisissez votre code")
            st.info(f"Code transmis à : **{st.session_state['user_email']}**")
            
            code_otp = st.text_input("Code à 6 chiffres", max_chars=8, placeholder="12345678")
            
            col_val, col_back = st.columns([1, 1])
            with col_val:
                if st.button("Valider et accéder 🚀", type="primary", width="stretch"):
                    if code_otp and len(code_otp.strip()) >= 6:
                        try:
                            ...
                            if res.session:
                                ...
                                # Transmet le token pour le RLS
                                supabase.postgrest.auth(res.session.access_token)
                                # 🔑 Calcul des rôles (Admin / Valideur)
                                est_admin, est_valideur = verifier_roles_utilisateur(res.user.email)
                        ...
                        st.warning("⚠️ Veuillez entrer les 8 chiffres du code.")

            with col_back:
                if st.button("Changer d'e-mail 🔄", width="stretch"):
                    st.session_state["otp_sent"] = False
                    st.rerun()
...
with onglet_nouvelle:
    ...
    mode_acces = st.radio("Véhicule ou piéton ?", ["Véhicule", "Piéton"], horizontal=True, label_visibility="collapsed")
    ...
    with st.form("form_demande_acces", clear_on_submit=True):
        ...
        if mode_acces == "Véhicule":
            ...
            type_vehicule = st.text_input("Type de véhicule (marque / modèle / couleur) *")
            st.divider()
        else:
            conducteur = None
            immat_nc = None
        ...
        with col_PMR:
            pmr_choix = st.radio("Personne à mobilité réduite ?", ["Oui", "Non"], horizontal=True)
            pmr_booleen = True if pmr_choix == "Oui" else False
            
        with col_personnes:
            nombre_personnes = st.number_input("Nombre de personnes *", min_value=1, value=1, step=1)

        col_orga, col_email = st.columns(2)
        with col_orga:
            organisme = st.text_input("Raison sociale ou organisme *")
        with col_email:
            # Pré-remplissage avec l'email de l'utilisateur connecté !
            email_demandeur = st.text_input("Email du demandeur *", value=st.session_state["user_email"])
        ...
        if submit:
            if site_choisi == "Sélectionnez un site...":
                ...
                st.error("⚠️ Veuillez remplir tous les champs obligatoires (Organisme, Email, Motif).")
            elif mode_acces == "Véhicule" and (not conducteur or not immatriculation or not type_vehicule):
                st.error("⚠️ Veuillez remplir toutes les informations liées au véhicule.")
            elif date_sortie < date_entree:
                st.error("⚠️ La date de sortie ne peut pas être antérieure à la date d'entrée.")
            else:
                email_du_valideur = None
                try:
                    req_valideur = supabase.table("Valideurs_sites").select("valideur_email").eq("site_nom", site_choisi).execute()
                ...
                immat_strict = normaliser_immatriculation(immatriculation, immat_nc) if mode_acces == "Véhicule" else None
                ...
                try:
                    reponse = supabase.table("Demandes_acces").insert(donnees_a_inserer).execute()
                    ...
                    if len(reponse.data) > 0:
                        ...
                        st.success(f"✅ Votre demande pour **{site_choisi}** a été soumise avec succès et transmise à **{email_du_valideur}** pour validation !")
                    else:
...
with onglet_historique:
    ...
    try:
        ...
        # 🔒 SÉCURITÉ : Un Admin voit tout, un Agent ne voit QUE SES DEMANDES
        if st.session_state.get("is_admin"):
            reponse_historique = supabase.table("Demandes_acces") \
                .select("*") \
                .order("id", desc=True) \
                .execute()
        else:
        ...
        if not donnees_demandes:
            st.warning("Vous n'avez encore soumis aucune demande d'accès avec cette adresse e-mail.")
        else:
            ...
            cols_existantes = [col for col in colonnes_a_afficher.keys() if col in df.columns]
            df_affiche = df[cols_existantes].rename(columns=colonnes_a_afficher)

            if "Date d'entrée" in df_affiche.columns:
                df_affiche["Date d'entrée"] = pd.to_datetime(df_affiche["Date d'entrée"]).dt.strftime('%d/%m/%Y')
            if "Date de sortie" in df_affiche.columns:
                df_affiche["Date de sortie"] = pd.to_datetime(df_affiche["Date de sortie"]).dt.strftime('%d/%m/%Y')
            ...
            with col_stat:
                ...
                st.caption(f"📊 **Statistiques :** ⏳ {nb_en_attente} En attente | ✅ {nb_valide} Validée(s) | ❌ {nb_refuse} Refusée(s)")
...
with onglet_actifs:
    ...
    st.info("💡 Seules les demandes validées dont la date de fin n'est pas dépassée s'affichent ici.")

    try:
        reponse_actifs = supabase.table("Demandes_acces").select("*").eq("statut", "Validé").order("id", desc=True).execute()
        ...
        if not donnees_actives:
            ...
            for i, acces in enumerate(donnees_actives):
                with cols[i % 3]:
                    ...
                    d_ent_fr = f"{d_ent[8:10]}/{d_ent[5:7]}/{d_ent[0:4]}" if d_ent and len(d_ent) >= 10 else "N/C"
                    
                    d_sor = acces['date_sortie']
                    d_sor_fr = f"{d_sor[8:10]}/{d_sor[5:7]}/{d_sor[0:4]}" if d_sor and len(d_sor) >= 10 else "N/C"
...
if onglet_valideur is not None:
    with onglet_valideur:
        ...
        try:
            # 1. Si l'utilisateur est un Admin, il voit toutes les demandes
            if st.session_state.get("is_admin"):
                reponse_attente = supabase.table("Demandes_acces").select("*").eq("statut", "En attente").order("id", desc=True).execute()
            
            # 2. Si c'est un Valideur standard, on filtre par les sites qui lui sont assignés
            else:
                email_connecte = st.session_state.get("user_email")
                # On récupère les sites assignés à ce valideur dans la table Valideurs_sites
                req_sites = supabase.table("Valideurs_sites").select("site_nom").eq("valideur_email", email_connecte).execute()
                sites_assignes = [ligne["site_nom"] for ligne in req_sites.data] if req_sites.data else []

                if sites_assignes:
                    reponse_attente = supabase.table("Demandes_acces").select("*").eq("statut", "En attente").in_("site_id", sites_assignes).order("id", desc=True).execute()
                else:
                    reponse_attente = type('obj', (object,), {'data': []})() # Liste vide si aucun site assigné

            demandes = reponse_attente.data

            if not demandes:
                st.success("🎉 Aucune demande en attente de validation pour vos sites pour le moment !")
            else:
                for demande in demandes:
                    ...
                    with st.expander(titre_carte, expanded=True):
                        ...
                        with col_info1:
                            ...
                            st.markdown(f"**Personnes :** {demande['nombre_personnes']} (PMR : {'Oui' if demande['pmr'] else 'Non'})")
                        
                        with col_info2:
                            st.markdown(f"**Mode d'accès :** {demande['mode_acces']}")
                            if demande['mode_acces'] == "Véhicule":
                                st.markdown(f"🚗 **Immatriculation :** {demande.get('vehicule_immatriculation', 'N/C')} ({demande.get('vehicule_type', 'N/C')})")
                                st.markdown(f"👤 **Conducteur :** {demande.get('vehicule_conducteur', 'N/C')}")
                        ...
                        with col_btn1:
                            if st.button("✅ Approuver la demande", key=f"valider_{demande['id']}", type="primary"):
                                supabase.table("Demandes_acces").update({"statut": "Validé"}).eq("id", demande['id']).execute()
                                ...
                                st.success("Accès validé et notification envoyée au demandeur !")
                                st.rerun()
                                
                        with col_btn2:
                            if st.button("❌ Refuser la demande", key=f"refuser_{demande['id']}"):
                                if not champ_refus.strip():
                                    st.error("⚠️ Veuillez indiquer un motif de refus avant de rejeter la demande.")
                                else:
                                    supabase.table("Demandes_acces").update({"statut": "Refusé"}).eq("id", demande['id']).execute()
                                    ...
                                    st.error("Accès refusé et notification envoyée au demandeur.")
                                    st.rerun()
...
if onglet_admin is not None:
    with onglet_admin:
        ...
        st.info("Espace réservé aux administrateurs pour configurer qui valide l'accès à quel bâtiment.")
        ...
        with col_form:
            ...
            with st.form("form_ajout_valideur", clear_on_submit=True):
                site_a_configurer = st.selectbox("Sélectionnez le site", sites_disponibles_admin)
                nouvel_email_valideur = st.text_input("Email du responsable (Valideur)", placeholder="prenom.nom@gouv.nc")
                
                submit_valideur = st.form_submit_button("Enregistrer le valideur", type="primary")
                
                if submit_valideur:
                    if site_a_configurer == "Sélectionnez un site..." or not nouvel_email_valideur:
                        st.error("Veuillez sélectionner un site et saisir un email.")
                    else:
                        try:
                            verification = supabase.table("Valideurs_sites").select("*").eq("site_nom", site_a_configurer).execute()
                            
                            if len(verification.data) > 0:
                                id_existant = verification.data[0]["id"]
                                supabase.table("Valideurs_sites").update({"valideur_email": nouvel_email_valideur}).eq("id", id_existant).execute()
                                st.success(f"🔄 Mise à jour réussie pour {site_a_configurer} !")
                            else:
                                donnees = {
                                    "site_nom": site_a_configurer,
                                    "valideur_email": nouvel_email_valideur
                                }
                                supabase.table("Valideurs_sites").insert(donnees).execute()
                                st.success(f"✅ Enregistrement réussi pour {site_a_configurer} !")
        ...
        with col_liste:
            ...
            try:
                reponse_valideurs = supabase.table("Valideurs_sites").select("*").execute()
                ...
                if reponse_valideurs.data:
                    ...
                    st.dataframe(df_valideurs, width="stretch", hide_index=True)
                else:
